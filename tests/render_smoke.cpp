...
int main()
{
    ...
    for (int i = 0; i < 16; ++i)
        synth.addVoice (new zw::ZWVoice (refs, wt, matrix, bpm));
    ...
    const bool ok = std::isfinite (rms) && std::isfinite (peak) && rms > 1.0e-4 && peak <= 4.0;
    std::printf ("%s\n", ok ? "PASS: synth produced sound" : "FAIL: silent or non-finite output");
    return ok ? 0 : 1;
}
